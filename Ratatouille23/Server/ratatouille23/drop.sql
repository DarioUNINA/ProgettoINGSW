
    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKb2n1ml8vii5v12i6mg87jpocw;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKb2n1ml8vii5v12i6mg87jpocw;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKebadei83xe3alnh24abh0u1cf;

    alter table if exists utente 
       drop constraint if exists FKb2n1ml8vii5v12i6mg87jpocw;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKebadei83xe3alnh24abh0u1cf;

    alter table if exists utente 
       drop constraint if exists FKb2n1ml8vii5v12i6mg87jpocw;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKb2n1ml8vii5v12i6mg87jpocw;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists elemento 
       drop constraint if exists FKk20drd2mimlfx536smq4ytlkb;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKpx7fwvwm6lv7kq66c0sdbpeb2;

    alter table if exists allergia 
       drop constraint if exists FK49o2goobyll4pxtm3hgwclm72;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKpx7fwvwm6lv7kq66c0sdbpeb2;

    alter table if exists allergia 
       drop constraint if exists FK49o2goobyll4pxtm3hgwclm72;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKpx7fwvwm6lv7kq66c0sdbpeb2;

    alter table if exists allergia 
       drop constraint if exists FK49o2goobyll4pxtm3hgwclm72;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKpx7fwvwm6lv7kq66c0sdbpeb2;

    alter table if exists allergia 
       drop constraint if exists FK49o2goobyll4pxtm3hgwclm72;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKpx7fwvwm6lv7kq66c0sdbpeb2;

    alter table if exists allergia 
       drop constraint if exists FK49o2goobyll4pxtm3hgwclm72;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKpx7fwvwm6lv7kq66c0sdbpeb2;

    alter table if exists allergia 
       drop constraint if exists FK49o2goobyll4pxtm3hgwclm72;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
<<<<<<< Updated upstream
=======
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
>>>>>>> Stashed changes
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists categoria_id_categoria_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;

    alter table if exists allergia 
       drop constraint if exists FKenetpo854cyk13m0lr2cx20pi;

    alter table if exists allergia 
       drop constraint if exists FKr3j1jcqupwur10tm2hmxc3vrd;

    alter table if exists categoria 
       drop constraint if exists FK7sl2hag5q6tjjgajdbqlq5blp;

    alter table if exists elemento 
       drop constraint if exists FK7c98cnfsldokgorotjgy5dk9w;

    alter table if exists menu 
       drop constraint if exists FKcn7cg1eeunjsl6bqan7d4svyc;

    alter table if exists ordinazione 
       drop constraint if exists FKa1r5uatnvcl0829vidygetrxs;

    alter table if exists ordinazione 
       drop constraint if exists FKqewrash8k8swjyiyld70i297v;

    alter table if exists ordine 
       drop constraint if exists FKeqk4wbgp4bm8pwdcqy32w5xxr;

    alter table if exists tavolo 
       drop constraint if exists FKauvtvp4ls2ijk7efgw5ytsbtw;

    alter table if exists tavolo 
       drop constraint if exists FKi070cwdmd6tj2sywp96itncns;

    alter table if exists utente 
       drop constraint if exists FKkq3kqqcspxtvi490xjqx62ipu;

    drop table if exists allergene cascade;

    drop table if exists allergia cascade;

    drop table if exists categoria cascade;

    drop table if exists elemento cascade;

    drop table if exists menu cascade;

    drop table if exists ordinazione cascade;

    drop table if exists ordine cascade;

    drop table if exists ristorante cascade;

    drop table if exists tavolo cascade;

    drop table if exists utente cascade;

    drop sequence if exists id_categoria_seq;

    drop sequence if exists id_elemento_seq;

    drop sequence if exists id_menu_seq;

    drop sequence if exists id_ordine_seq;

    drop sequence if exists id_ristorante_seq;

    drop sequence if exists id_tavolo_seq;
